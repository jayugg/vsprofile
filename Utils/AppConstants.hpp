...
namespace vsprofile::constants {
    ...
    inline constexpr std::string_view kAppVersion = VSPROFILE_VERSION;
    ...
    inline const fs::path kAppDataDir = [] {
        ...
        if (home) {
            return fs::path(home) / "Library" / "Application Support"; // Use this because Vintage Story stores it here on Mac for v > 1.21.0-pre.2
        }
#else // Linux / Unix
